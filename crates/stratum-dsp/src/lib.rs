...
pub fn analyze_audio(
    ...
    // Local vendored patch: `std::time::Instant` is unimplemented on
    // wasm32-unknown-unknown, which panicked the analysis worker. The
    // timing feeds only the informational processing_time_ms field, so
    // on wasm we report 0.0 instead of panicking.
