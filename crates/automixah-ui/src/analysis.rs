//! In-memory analysis cache: content hash → detected beat grid.
//!
...
/// Shared, thread-safe analysis cache (data service: `Arc` of plain data).
#[derive(Debug, Clone, Default)]
pub struct AnalysisCache {
    grids: Arc<RwLock<HashMap<TrackHash, BeatGrid>>>,
}
...
impl AnalysisCache {
    ...
    pub fn get(&self, hash: &TrackHash) -> Option<BeatGrid> {
        self.grids.read().get(hash).cloned()
    }

    /// Caches the detected `grid` for `hash`.
    pub fn put(&self, hash: TrackHash, grid: BeatGrid) {
        self.grids.write().insert(hash, grid);
    }

    /// Drops the cached analysis for `hash` (re-analyze path).
    pub fn invalidate(&self, hash: &TrackHash) {
        self.grids.write().remove(hash);
    }
}
